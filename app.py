...
SPECIAL_PAIRS = {
    ...
    ("VINEETH", "MLA"),
    ("MADHAV", "TANAYA"),
    ("NEW_NAME1", "NEW_NAME2"), ("NEW_NAME2", "NEW_NAME1")
}
...
SPECIAL_CASES = {
    ...
    ("NEW_NAME1", "NEW_NAME2"): ("75%", "This is a new special case!"),
    ("NEW_NAME2", "NEW_NAME1"): ("25%", "This is another new special case!")
}
...
def show_loading_animation():
    ...
    for percent in range(0, 101, 10):
        ...
        animation_text.markdown(
            f"<h2 style='text-align: center;'>{random.choice(hearts)}</h2>",
...
st.markdown(
    """
    <div style='text-align: center; background: linear-gradient(to right, #ff416c, #ff4b2b);
...
if st.button("Calculate Love ❤"):
    if not name1 or not name2:
        ...
        st.markdown(
            f"<h2 style='text-align: center; color: black;'>Love Percentage: {result}</h2>",
            unsafe_allow_html=True
        )
        ...
        if result == "100%":
            ...
            st.image(
                "https://www.shutterstock.com/image-illustration/3d-better-luck-next-time-600nw-2504793501.jpg",
                caption="Better Luck Next Time!",
                use_container_width=True
            )
            funny_messages = [
                "Maybe try chocolates first? 🍫",
